...
async function createRequest(clientUid, payload, clientInfo = {}) {
    ...
    const existingClientRequestsSnap = await db.collection(REQUESTS_COLLECTION)
        .where('clientId', '==', clientUid)
        .limit(25)
        .get();

    const activeRequestCount = existingClientRequestsSnap.docs.reduce((count, snap) => {
        const data = snap.data() || {};
        const normalizedStatus = typeof data.status === 'string' ? data.status.trim().toUpperCase() : '';
        return TERMINAL_STATUSES.includes(normalizedStatus) ? count : count + 1;
    }, 0);

    if (activeRequestCount >= CLIENT_ACTIVE_REQUEST_LIMIT) {
        throw appError(
            409,
            'request_limit_reached',
            `Ya tienes ${CLIENT_ACTIVE_REQUEST_LIMIT} solicitudes activas. Cierra o completa una antes de crear otra.`,
        );
    }
