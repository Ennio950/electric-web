...
import { loadEmployeeEmergencyCalls } from '@/src/services/apiService';
...
export default function EmployeeEmergencyQueueScreen() {
  ...
  const emergencyQuery = useQuery({
    queryKey: ['employee-emergency-calls'],
  ...
  const calls = emergencyQuery.data ?? [];
  ...
  return (
    ...
    >
      ...
      <SectionCard title="Pendientes" subtitle="Emergencias abiertas disponibles para aceptar.">
        {emergencyQuery.isLoading ? <LoadingScreen variant="skeleton" /> : null}
        {emergencyQuery.error ? (
          <Text style={styles.error}>
            {emergencyQuery.error instanceof Error ? emergencyQuery.error.message : 'No se pudo cargar la cola.'}
          </Text>
        ) : null}
        {pendingCalls.length ? (
        ...
        {!emergencyQuery.isLoading && !emergencyQuery.error && !pendingCalls.length ? (
          <EmptyState
            icon="✅"
      ...
      <SectionCard title="Asignadas a mi" subtitle="Emergencias ya tomadas por tu usuario.">
        ...
        {!emergencyQuery.isLoading && !emergencyQuery.error && !assignedCalls.length ? (
          <EmptyState
            icon="🚨"
